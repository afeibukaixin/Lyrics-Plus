...
- **播放器：** Apple Music、Spotify 和兼容的系统媒体应用；支持播放控制、来源过滤、播放器跟随启动与静默启动。
...
- **封面：** 优先使用播放器封面，可选 Cover Art Archive 与 iTunes 在线补全、地区选择、高清替换和缓存管理。
- **应用能力：** 菜单栏、全局快捷键、快速切换歌词、JSONC 配置、实时日志、四种界面语言与主动确认更新。
...
启用在线歌词或封面补全时，歌曲名、歌手、专辑和时长等匹配信息会发送给所选第三方服务。相关内容、可用性、授权范围和数据处理规则不受本项目控制。

歌词和封面权利归相应权利人所有。Lyrics Plus 仅提供搜索、解析、缓存、导入和展示功能，与 Apple Music、Spotify、歌词服务及权利人不存在隶属或背书关系。软件按 MIT License 以现状提供。
