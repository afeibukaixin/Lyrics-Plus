...
除了 Apple Music 和 Spotify 的专用集成外，系统媒体模式还可以跟随通过 macOS 系统媒体通道提供 Now Playing 元数据的第三方音乐应用。在“设置 → 应用”中选择“仅使用系统媒体”，还可以选择性地添加第三方应用到允许列表。具体能否读取元数据以及执行播放控制，取决于音乐应用实际提供的能力。
