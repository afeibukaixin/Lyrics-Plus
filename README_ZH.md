...
Lyrics Plus 会自动同步 Apple Music、Spotify，或通过系统媒体通道提供兼容信息的第三方 macOS 音乐应用的当前歌曲、播放状态和进度，并在主窗口与桌面浮窗中展示同步歌词。当前功能包含多歌词源、翻译与音译、逐词时间轴、歌词库、在线封面补全以及高度可定制的桌面歌词。
...
| 歌词获取 | 来源排序、启停、连接测试、标题过滤与繁简匹配 | ✅ |
| 歌词获取 | 本地 LRC 导入与歌曲关联 | ✅ |
| 歌词管理 | 歌词库、离线缓存与同步偏移调整 | ✅ |
| 歌词内容 | 同步歌词、翻译与音译 | ✅ |
| 歌词内容 | 逐词时间轴 | ✅ |
| 封面 | 播放器封面优先与在线封面补全 | ✅ |
| 封面 | Cover Art Archive / iTunes 来源、地区选择、测试、排序与缓存管理 | ✅ |
| 桌面歌词 | 透明、纯色与玻璃背景 | ✅ |
| 桌面歌词 | 横排、竖排、单排与双排布局 | ✅ |
| 桌面歌词 | 逐词扫光、弹跳与高亮效果 | ✅ |
...
还可以选择一个系统媒体应用：当它正在运行时自动跟随启动 Lyrics Plus；静默启动会让应用先在后台运行，直到你需要打开主窗口。

## 在线封面补全

始终优先使用播放器提供的封面。开启后，只有封面缺失、原图短边小于 256px，或某个系统媒体应用被设为始终尝试在线封面（适用于尺寸足够但实际模糊的封面）时，才会查询第三方来源。

Cover Art Archive 和 iTunes 可以单独启停、测试和调整顺序。iTunes 支持中国大陆、台湾、香港和美国商店地区；自动模式跟随应用语言。封面会尽可能按专辑缓存；可在“设置 → 封面”查看缓存、变更目录，或仅清空 Lyrics Plus 管理的缓存文件。
...
从 v1.0.2 开始的正式安装版默认会在启动时检查更新。发现新版本后，Lyrics Plus 会询问是否下载和安装，不会静默更新；可在“设置 → 关于与更新”管理自动检查或手动检查。开发环境不检查更新。
...
启用在线歌词源或在线封面补全时，歌曲标题、歌手、专辑和时长等用于匹配的信息会发送给相应的第三方服务。封面匹配可能访问 Cover Art Archive、MusicBrainz 和 iTunes；其内容、可用性、准确性、授权范围和数据处理规则由服务提供方决定。
