...
export default function LyricsSettingsPage() {
  ...
  const {
    ...
    saveProviderSettings, resetProviderSetting, saveMusixmatchToken, clearMusixmatchToken,
  ...
  const resetTitleFilters = async () => {
    if (savingTitleFilters) return;
    setSavingTitleFilters(true);
    await resetProviderSetting("titleFilterKeywords");
    setSavingTitleFilters(false);
  };
  ...
  const resetAmllBaseUrl = async () => {
    if (savingProviderConfig) return;
    setSavingProviderConfig(true);
    const saved = await resetProviderSetting("amllBaseUrl");
    setSavingProviderConfig(false);
    if (saved) {
      setAmllBaseUrlDraft(defaultAmllBaseUrl);
      setProviderConfig(null);
    }
  };
  ...
  return <SettingsPage sections={sections}>
    ...
    <SettingsSection id="lyrics-title-filters" title={t("settings.lyrics.titleFilters")} trailing={<Button variant="ghost" size="sm" disabled={savingTitleFilters} onClick={() => void resetTitleFilters()}>{t("settings.lyrics.restoreTitleFilters")}</Button>}>
    ...
    <Dialog open={providerConfig !== null} onOpenChange={(open) => { if (!open && !savingProviderConfig) setProviderConfig(null); }}>
      <DialogContent className="max-h-[calc(100vh-2rem)] overflow-y-auto">
        {providerConfig === "musixmatch" ? <form onSubmit={(event) => void saveMusixmatch(event)}>
          ...
          <DialogFooter>
            <Button type="button" variant="ghost" disabled={savingProviderConfig} onClick={() => void resetAmllBaseUrl()}>{t("common.actions.resetDefault")}</Button>
