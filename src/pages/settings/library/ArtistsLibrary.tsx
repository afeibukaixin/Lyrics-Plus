...
import { LibraryDetailHeader, LibraryDetailSection, LibraryRelationItem, LibraryRelationList, LibraryState, LibraryToolbar, PageControls, TruncatedText } from "./shared";
...
export default function ArtistsLibrary({ detailId }: { detailId: number | null }) {
  ...
  if (detailId) return <ArtistDetail artistId={detailId} onBack={() => navigate("/settings/library/artists")} />;
  return (
    <div className={styles.workspace}>
      <Card className={`${styles.panel} ${styles.listPanel}`}>
        ...
        {error ? (
          ...
          <>
            {data.items.length ? (
              <Table className={`${styles.fixedTable} ${styles.artistsTable} ${styles.dataTable}`}>
                <TableHeader><TableRow><TableHead>{t("library.manager.artist")}</TableHead><TableHead>{t("library.manager.aliases")}</TableHead><TableHead className={styles.numericColumn}>{t("library.manager.songCount")}</TableHead><TableHead>{t("library.manager.rawCredits")}</TableHead><TableHead className={styles.actionColumn}>{t("library.manager.actions")}</TableHead></TableRow></TableHeader>
                <TableBody>{data.items.map((item) => <TableRow key={item.artistId}><TableCell className={styles.primaryCell}><TruncatedText variant="title">{item.canonicalName}</TruncatedText></TableCell><TableCell>{item.aliases.length ? <div className={styles.tableAliasBadges}>{item.aliases.map((value) => <Badge className={styles.tableAliasBadge} variant="secondary" key={value}><TruncatedText>{value}</TruncatedText></Badge>)}</div> : "—"}</TableCell><TableCell className={styles.numericColumn}><TruncatedText>{String(item.songCount)}</TruncatedText></TableCell><TableCell><TruncatedText>{item.rawNames.join(" / ") || "—"}</TruncatedText></TableCell><TableCell className={styles.actionColumn}><Button size="sm" variant="outline" onClick={() => navigate(`/settings/library/artists/${item.artistId}`)}>{t("library.manager.details")}</Button></TableCell></TableRow>)}</TableBody>
              </Table>
            ) : (
...
function ArtistDetail({ artistId, onBack }: { artistId: number; onBack: () => void }) {
  const { t } = useTranslation();
  const navigate = useNavigate();
