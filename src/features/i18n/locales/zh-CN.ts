export const zhCN = {
  ...
  player: {
    ...
    sourceNotAllowed: "{{application}}（{{bundleId}}）不在自动识别允许列表中",
  },
  home: {
...
failed: "歌词目录扫描失败",
  ...
  settings: {
    ...
    app: {
      ...
      playerMode: "播放方式", playerAuto: "智能选择", playerSystem: "仅使用系统媒体", playerHint: "推荐使用智能选择；识别异常时再手动指定。", playerFollower: "跟随播放器启动和退出", playerFollowerHint: "单独选择一个播放器作为生命周期目标；播放器启动时打开 Lyrics Plus，退出时关闭 Lyrics Plus。", playerFollowerEmpty: "未启用播放器跟随", choosePlayerFollower: "选择播放器", playerFollowerPicker: "播放器应用", playerFollowerDevelopment: "开发模式下已禁用；安装后的正式应用中可用。", playerFollowerUnsupported: "当前系统不支持播放器跟随。", playerFollowerNotFound: "播放器跟随 Helper 未正确安装。", playerFollowerNotRegistered: "播放器跟随服务尚未注册。", retryPlayerFollower: "重试", playerFollowerApproval: "后台项目已被系统停用，请在登录项设置中允许 Lyrics Plus。", openLoginItems: "打开登录项设置", systemApplications: "系统播放应用", systemApplicationsHint: "仅用于系统媒体通道；不包含 Apple Music 和 Spotify，留空则不限制第三方来源。", systemApplicationsEmpty: "尚未添加系统播放应用", addCurrentApplication: "添加当前播放应用", chooseApplications: "从访达选择应用", systemApplicationsPicker: "macOS 应用", display: "主界面显示", language: { label: "界面语言", description: "立即应用到所有窗口，并随应用配置保存。" },
