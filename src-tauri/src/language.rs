...
impl UiLanguage {
    ...
    pub fn config_comment(self, comment: ConfigComment) -> &'static str {
        match (self, comment) {
            ...
            (Self::ZhCn, ConfigComment::SystemMediaApplications) => "系统媒体通道允许的第三方应用；空数组表示全部允许。",
            (Self::ZhCn, ConfigComment::HideDockIcon) => "隐藏 macOS 程序坞图标；仍可从菜单栏使用 Lyrics Plus。",
            (Self::ZhCn, ConfigComment::SilentStartup) => "启动时不显示主界面；仍可从菜单栏打开 Lyrics Plus。",
            ...
            (Self::EnUs, ConfigComment::SystemMediaApplications) => "Third-party apps allowed through the System Media channel; an empty list allows all.",
            (Self::EnUs, ConfigComment::HideDockIcon) => "Hide the macOS Dock icon; Lyrics Plus remains available from the menu bar.",
            (Self::EnUs, ConfigComment::SilentStartup) => "Start without showing the main window; Lyrics Plus remains available from the menu bar.",
