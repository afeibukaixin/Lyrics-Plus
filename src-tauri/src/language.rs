...
pub enum ConfigComment {
    ...
    Shortcuts,
    AutoApplyThreshold,
    AutoApplyDurationGuard,
    AutoApplyDurationTolerance,
    AutoSearchDebounce,
    TitleFilterKeywords,
    AmllBaseUrl,
    ProviderMode,
...
impl UiLanguage {
    ...
    pub fn config_comment(self, comment: ConfigComment) -> &'static str {
        match (self, comment) {
            ...
            (Self::ZhCn, ConfigComment::AutoApplyDurationGuard) => "是否检查歌词时间轴末尾超过歌曲时长；关闭后跳过该自动采用保护。",
            (Self::ZhCn, ConfigComment::AutoApplyDurationTolerance) => "歌词时间轴允许超过歌曲时长的秒数：0–60；仅影响自动采用。",
            ...
            (Self::EnUs, ConfigComment::AutoApplyDurationGuard) => "Whether to reject automatic lyrics whose timeline extends beyond the track duration; disabling skips this guard.",
            (Self::EnUs, ConfigComment::AutoApplyDurationTolerance) => "Seconds by which the lyric timeline may exceed the track duration: 0–60; automatic matching only.",
