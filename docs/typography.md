...
新增或调整普通 UI 文本时，按以下参考把常见字号映射到现有语义 Token：

| 常见字号 | 主要语义 | 推荐 Token |
| --- | --- | --- |
| `.4375rem`、`.46875rem`、`.5rem` | 极弱说明、时间、计数 | `caption`；过小用法应同时校正可读性 |
...
当前显式行高主要为 `1`、`1.2`、`1.45`、`1.5`、`1.55`、`1.6`，字重主要为 `550`、`600`、`750`、`800`。普通 UI 优先采用 Token 推荐值；单行截断或紧凑控件可保留 `line-height: 1`，品牌标签及关键状态可保留较高字重，但需要在使用处说明语义。

## 当前实现

设置页、快速歌词窗口、调试日志和配置编辑器中的普通 UI 固定字号均已引用语义化 Token，并同时引用相应行高与推荐字重。`body` 提供 `body` 默认层级，`button`、`input`、`select` 和 `textarea` 统一继承字体属性；技术文本统一引用等宽字体 Token。
...
- 桌面歌词工具栏：工具栏是浮层内的紧凑操作区，目前 `8px`、`9px` 属于受浮层尺寸约束的例外；若调整工具栏尺寸或交互密度，再单独复核可读性。
